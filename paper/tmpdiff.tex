...
%DIF DEL old.tex        Mon Jan 15 10:03:48 2018
%DIF ADD deadtime.tex   Mon Jan 15 14:54:03 2018
...
\DIFdelbegin \DIFdel{The periodogram, commonly referred to as }\DIFdelend \DIFaddbegin \DIFadd{Dead time heavily distorts the periodogram, the most widely used statistical tool to estimate the }\DIFaddend power density spectrum (PDS)%
...
, \DIFdelbegin \DIFdel{which is the most widely used statistical tool to investigate rapid variability, is heavily distorted by dead time, }\DIFdelend with a characteristic pattern \DIFdelbegin \DIFdel{similar to a damped oscillator.
This pattern }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend is stronger for brighter sources\DIFdelbegin \DIFdel{, and it }\DIFdelend \DIFaddbegin \DIFadd{. 
It }\DIFaddend is often not possible to disentangle this power spectral distortion due to dead time and the broadband noise components characterizing the emission of accreting systems.
...
\footnote{\href{https://github.com/matteobachetti/deadtime-paper-II/}{https://github.com/matteobachetti/deadtime-paper-II/}}%DIF > 
\footnote{\href{https://github.com/stingraysoftware/HENDRICS/notebooks}{https://github.com/stingraysoftware/HENDRICS/notebooks}}\DIFdelbegin \DIFdel{\citep[for a description of Jupyter notebooks, see][]{kluyver2016jupyter}}\DIFdelend .
...
\caption{Periodogram and cospectrum, before and after FAD correction, for a pure white noise light curve \DIFaddbeginFL \DIFaddFL{(count rate 2000 ct/s)}\DIFaddendFL . 
...
\DIFdelbeginFL \DIFdelFL{Spectra were }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{We averaged 500 periodograms }\DIFaddendFL calculated over 2-sec intervals\DIFdelbeginFL \DIFdelFL{and averaged}\DIFdelendFL , to decrease the scatter and highlight the distortion of powers.}
...
Let us simulate two constant \DIFaddbegin \DIFadd{1000-sec }\DIFaddend light curves with an incident mean count rate of \DIFdelbegin \DIFdel{400 }\DIFdelend \DIFaddbegin \DIFadd{2000 }\DIFaddend counts/sec and a dead time of 2.5 ms, as we would expect from the two identical detectors of \nustar observing the same stable X-ray source.
...
Also, it reinstates a correct distribution of powers, following \DIFaddbegin \DIFadd{very closely }\DIFaddend the expected $\chi^2_2$ distribution \DIFdelbegin \DIFdel{\citep{Lewin+88} very closely}\DIFdelend \DIFaddbegin \DIFadd{(\citealt{Lewin+88}; Figure~\ref{fig:dist}, right)}\DIFaddend . 
Analogously, the corrected cospectrum will follow the expected Laplace distribution \DIFdelbegin \DIFdel{\citep{HuppenkothenBachetti18}}\DIFdelend \DIFaddbegin \DIFadd{(\citealt{HuppenkothenBachetti18}; Figure~\ref{fig:dist}, left)}\DIFaddend .
While the original dead time-affected cospectrum had a \DIFaddbegin \DIFadd{distorted, }\DIFaddend frequency-dependent \DIFdelbegin \DIFdel{modification to the }\DIFdelend \rms level, the FAD-corrected cospectrum \DIFdelbegin \DIFdel{gets back to a frequency-independent shape, like in the dead time-free case}\DIFdelend \DIFaddbegin \DIFadd{returns to the correct distribution at all frequencies}\DIFaddend .
...
\item for each pair of light curve segments: 
	...
	\item \textit{smooth} the FAD using a Gaussian-window interpolation with a large number of bins, in our case all the bins contained in 1-2 Hz\DIFaddbegin \DIFadd{, but this might need an adjustment at extreme count rates ($\gtrsim 10/\deadt$), where significant gradients in the white noise periodogram can occur in less than 1 Hz}\DIFaddend ;
...
\acknowledgments
We thank the anonymous referee for providing very \DIFdelbegin \DIFdel{helpful }\DIFdelend \DIFaddbegin \DIFadd{insightful }\DIFaddend feedback.
...
\DIFdelbegin %DIFDELCMD < \software{Astropy \citep{astropy2013},
%DIFDELCMD <           Matplotlib \citep{Matplotlib07},
%DIFDELCMD <           scipy \& numpy \citep{numpy11},
%DIFDELCMD <           stingray \citep{huppenkothen2016},
%DIFDELCMD <           HENDRICS \citep[before name change]{2015ascl.soft02021B}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \software{Astropy \citep{astropy2013},
