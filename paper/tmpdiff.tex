...
%DIF DEL old.tex        Fri Jan 12 21:59:46 2018
%DIF ADD deadtime.tex   Fri Jan 12 22:16:45 2018
...
Whereas a noise cospectrum in the absence of dead time would follow a Laplace distribution \DIFdelbegin \DIFdel{(Huppenkothen and Bachetti,
sub.),
}\DIFdelend \DIFaddbegin \DIFadd{\citep{HuppenkothenBachetti18},
}\DIFaddend dead time affects the width of the probability distribution for cospectral powers and modulates the measured \rms similarly to the distortion acted on power spectra.
In this Letter, we show a method to precisely recover the shape of the \DIFdelbegin \DIFdel{power density spectrum }\DIFdelend \DIFaddbegin \DIFadd{periodogram }\DIFaddend by looking at the difference of the Fourier amplitudes of the light curves of two independent detectors.
...
In \sref{sec:fourierdiff} we show that, in the absence of dead time, the difference of the Fourier amplitudes calculated \DIFaddbegin \DIFadd{from }\DIFaddend two independent detectors \DIFdelbegin \DIFdel{contain }\DIFdelend \DIFaddbegin \DIFadd{contains }\DIFaddend the sum of the correlated signal (the source signal) and uncorrelated noise (detector-related noise), and that their difference eliminates the source part. 
...
%DIF < 
%DIF < 
%DIF < The resulting Fourier amplitudes will be of the form
%DIF < 
%DIF < \begin{eqnarray}
%DIF < A_{xj} &=& A_{xsj} + A_{xdj} + A_{xnj} \nonumber \\
%DIF < B_{xj} &=& B_{xsj} + B_{xdj} + B_{xnj} \, ,
%DIF < \end{eqnarray}
%DIF < 
...
%DIF < For a large enough number of data points $N$, the Fourier amplitudes $A_{xj}$ and $B_{xj}$ will be composed of a sum of three independent random normal variables, with $A_{xsj} \sim \Normal(0, \sigma_{sj}^2)$,  $A_{xdj} \sim \Normal(0, \sigma_{dj}^2)$ and $A_{xnj} \sim \Normal(0, \sigma_n^2)$, where $\sigma_{sj}^2 = \sigma_{s}^2(\nu) = N_\mathrm{phot}P(\nu)/4$ is given by the power spectrum of the underlying stochastic process, $P_j = P(\nu_j)$, $\sigma_{dj}^2$ is an unknown, frequency-dependent variance introduced by dead time, and $N_{\mathrm{phot}} = \sum_{k=1}^{N}{x_k}$ is the integrated flux in the light curve. We also have $\sigma_n^2 = N_\mathrm{phot}/2$, and hence the combined distributions become
%DIF < 
%DIF < \begin{eqnarray}
%DIF < A_{xj} &\sim & \Normal(0, \sigma_{sj}^2 + \sigma_{dj}^2 + \sigma_{n}^2) \nonumber \\
%DIF < A_{yj} &\sim & \Normal(0, \sigma_{sj}^2 + \sigma_{dj}^2 + \sigma_{n}^2) \nonumber \, .
%DIF < \end{eqnarray}
%DIF < 
...
%DIF < 
%DIF < As depicted in Figure~\ref{fig:fourierdiff}, the correlation between Fourier amplitudes implies that their difference will be independent of the source-induced variability $P(\nu_j)$ and will again be distributed following a normal distribution 
%DIF < 
%DIF < \[
%DIF < A_{xj} - A_{yj} \sim \Normal(0, 2\sigma_{dj}^2 + 2\sigma_{n}^2) \; .
%DIF < \]
%DIF < 
...
All simulated \DIFdelbegin \DIFdel{and real data }\DIFdelend sets in this paper were produced and \DIFdelbegin \DIFdel{/or }\DIFdelend analyzed with a combination of the two Python libraries \texttt{stingray}%
...
\citep{huppenkothen2016} and \texttt{HENDRICS} v.\texttt{3.0b2} \citep[formerly known as MaLTPyNT;][]{2015ascl.soft02021B}, both \DIFdelbegin \DIFdel{based on Astropy \citep{astropy2013}}\DIFdelend \DIFaddbegin \DIFadd{affiliated to Astropy \citep{astropy2013,astropy18}}\DIFaddend .

We used the same procedure and algorithms described by \citet{Bachetti+15}, Section 4, which we briefly summarize here.
%DIF < The classes in the \texttt{stingray.simulate} module were used to simulate light curves with a given noise profile. 
...
%DIF < We first created the noisy light curves and then used an acceptance-rejection method to obtain simulated events. 
...
Then, the \texttt{hendrics.fake.filter\_for\_deadtime()} function was used to apply a non-paralyzable dead time of 2.5\,ms to the simulated event lists. For more details on the simulated data sets \DIFaddbegin \DIFadd{and the validation of the simulation infrastracture}\DIFaddend , see also Section \ref{sec:correction} and the available Jupyter notebooks%
\footnote{\DIFdelbegin %DIFDELCMD < \href{https://github.com/matteobachetti/deadtime-paper-II}{https://github.com/matteobachetti/deadtime-paper-II}%%%
\DIFdelend \DIFaddbegin \href{https://github.com/matteobachetti/deadtime-paper-II/}{https://github.com/matteobachetti/deadtime-paper-II/}\DIFaddend }
\DIFaddbegin \footnote{\href{https://github.com/stingraysoftware/HENDRICS/notebooks}{https://github.com/stingraysoftware/HENDRICS/notebooks}}
\DIFaddend \citep[for a description of Jupyter notebooks, see][]{kluyver2016jupyter}.
After producing these synthetic event lists, we started the standard timing analysis: we produced light curves with a bin time of \DIFdelbegin \DIFdel{$\sim0.122$}\DIFdelend \DIFaddbegin \DIFadd{$\sim0.244$}\DIFaddend \,ms, and calculated power spectral products (cospectrum, periodogram) over segments of these light curves using \texttt{stingray}.
...
Analogously, the corrected cospectrum will follow the expected Laplace distribution  \DIFdelbegin \DIFdel{(Huppenkothen \& Bachetti, sub.
).
}\DIFdelend \DIFaddbegin \DIFadd{\citep{HuppenkothenBachetti18}.
}\DIFaddend While the original dead time-affected cospectrum had a frequency-dependent modification to the \rms level, the FAD-corrected cospectrum gets back to a frequency-independent shape, like in the dead time-free case.
...
\begin{figure*}
    ...
    \caption{Periodogram\DIFaddbeginFL \DIFaddFL{, in fractional }\rms \DIFaddFL{normalization, }\DIFaddendFL from a simulation with four Lorentzian features (at 50, 200, 300 and 400 Hz) with 40-Hz full with at half maximum (FWHM). 
...
We are now ready to verify \DIFdelbegin \DIFdel{the last step: is the }\DIFdelend \DIFaddbegin \DIFadd{whether the }\DIFaddend FAD-corrected periodogram \DIFdelbegin \DIFdel{equivalent (albeit with some loss of sensitivity due to the lower number of photons) to the }\DIFdelend \DIFaddbegin \DIFadd{is a good approximation to the }\DIFaddend dead time-free periodogram\DIFdelbegin \DIFdel{?
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend To test this, we produced a number of different synthetic datasets as explained in Section~\ref{sec:wndeadtime}, containing different combinations of QPOs and broadband noise components\DIFdelbegin \DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{, expressed as pairs of Lorentzian curves}\DIFaddend \footnote{See the notebooks at \href{https://github.com/matteobachetti/deadtime-paper-II}{https://github.com/matteobachetti/deadtime-paper-II} to reproduce the analysis plotted in the Figures of this paper and more. The algorithm described in Section~\ref{sec:fad} is contained in the \texttt{fad\_correction.py} file in the notebooks directory.}
We first calculated the periodogram and cospectrum of the dead time-free data, averaged over \DIFdelbegin \DIFdel{128-s }\DIFdelend \DIFaddbegin \DIFadd{64--512\,s }\DIFaddend intervals.
Then, we applied a \DIFaddbegin \DIFadd{2.5\,ms }\DIFaddend dead time filter to the event list and applied the FAD correction, as described in Section~\ref{sec:fad}

All spectra were then expressed in fractional \rms normalization \DIFdelbegin \DIFdel{, where the integral of the fitted power spectral components over the frequency returns directly its fractional }%DIFDELCMD < \rms%%%
\DIFdel{.
In the }%DIFDELCMD < \rms %%%
\DIFdelend \DIFaddbegin \DIFadd{\citep{Miyamoto+91,BelloniHasinger90}.
In this }\DIFaddend normalization, the \DIFdelbegin \DIFdel{values of each point of the periodogram, after white noise subtraction, should be consistent between }\DIFdelend \DIFaddbegin \DIFadd{integrated model returns the full fractional }\rms \DIFadd{of }\DIFaddend the \DIFaddbegin \DIFadd{light curve, and the }\DIFaddend dead time-free and the FAD-corrected periodograms \DIFdelbegin \DIFdel{.
We fitted all spectra with the model which produced the simulated data and checked if the }%DIFDELCMD < \rms %%%
\DIFdel{values were consistent with the fit on the dead time-free periodogram.
To calculate this }%DIFDELCMD < \rms%%%
\DIFdel{, we fitted the spectra with two Lorentzian components, and integrated the model }\DIFdelend \DIFaddbegin \DIFadd{should be consistent }\DIFaddend over the full frequency range.
\DIFdelbegin \DIFdel{For periodograms, the model included also a constant offset to account for the white noise level.
}\DIFdelend An example of this analysis is shown in Figure~\ref{fig:pds}: the FAD successfully corrects so well periodograms and cospectra when compared to dead-time free simulated spectra, that in the figure they are almost indistinguishable.

\DIFdel{In order to test the robustness of the FAD, we have }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend ran extensive simulations testing how the method performs (1) for a range of different input count rates, leading to dead-time effects of different magnitude in the output periodograms and cospectra, and (2) \DIFdelbegin \DIFdel{whenthe }\DIFdelend \DIFaddbegin \DIFadd{when the }\DIFaddend light curves do not have the same count rate (since detectors may in reality have slightly different efficiencies).
\DIFdelbegin \DIFdel{The simulations }\DIFdelend \DIFaddbegin \DIFadd{We fitted all spectra with a two-Lorentzian model, plus a constant offset to account for the white noise level in periodograms.
We calculated the }\rms \DIFadd{by integrating the model fitted above over the full frequency range, and compared the results in the deadtime-free and FAD-corrected cases.
}

\subsection{\DIFadd{Simulation results and discussion}}\label{sec:caveat}
...
\DIFadd{The simulations described above }\DIFaddend show that the shape of the periodogram is precisely corrected by the FAD procedure if \DIFdelbegin \DIFdel{(1) }\DIFdelend \textit{the input light curves have the same count rate} and \DIFdelbegin \DIFdel{(2) }\DIFdelend \textit{for values of the input count rate and \rms that are not too extreme}. Differing input count rates in different detectors matter in practice only for the single-detector periodogram, but \textit{not} for cospectra and total-intensity periodograms. 
At high count rates, single-detector periodograms are corrected very well only if the two detectors have very similar count rates, and count rates must be more similar \DIFdelbegin \DIFdel{athigher }\DIFdelend \DIFaddbegin \DIFadd{at higher }\DIFaddend count rates in order for the correction to apply.
...
%DIF < In these case, we provide some example simulation code to estimate this effect in the notebooks.
%DIF < This software will be merged into the main repository of \texttt{stingray} before publication.
...
\DIFaddbegin \software{Astropy \citep{astropy2013},
...
\DIFaddend \begin{thebibliography}{}
...
{Astropy Collaboration}, {\DIFdelbegin \DIFdel{Robitaille}\DIFdelend \DIFaddbegin \DIFadd{et~al.}\DIFaddend } \DIFdelbegin \DIFdel{, T.~P.}\DIFdelend \DIFaddbegin \DIFadd{2013,
  }\aap\DIFadd{, 558}\DIFaddend , \DIFaddbegin \DIFadd{A33
}
...
\DIFadd{Astropy Project, }\DIFaddend {\DIFdelbegin \DIFdel{Tollerud}%DIFDELCMD < }%%%
\DIFdel{, E.~J., }%DIFDELCMD < {%%%
\DIFdelend et~al.} \DIFdelbegin \DIFdel{2013, }%DIFDELCMD < \aap%%%
\DIFdel{,
  558, A33
}\DIFdelend \DIFaddbegin \DIFadd{2018, arXiv,
  arXiv:1801.02634
}\DIFaddend 
...
\DIFaddbegin \bibitem[{Hunter(2007)}]{Matplotlib07}
\DIFadd{Hunter, J.~D. 2007, Computing in Science and Engineering, 9, 90
}

\bibitem[{Huppenkothen \& Bachetti(2017)}]{HuppenkothenBachetti18}
\DIFadd{Huppenkothen, D., \& Bachetti, M. 2017, arXiv, arXiv:1709.09666
}

\DIFaddend \bibitem[{{Huppenkothen} {et~al.}(2016){Huppenkothen}, {Bachetti}, {Stevens},
...
\bibitem[{van~der Walt {et~al.}(2011)van~der Walt, Colbert, \&
  Varoquaux}]{numpy11}
\DIFadd{van~der Walt, S., Colbert, S.~C., \& Varoquaux, G. 2011, Computing in Science
  Engineering, 13, 22
}\DIFaddend 
