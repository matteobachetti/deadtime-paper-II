...
We first created the light curves and then used an acceptance-rejection method to obtain simulated events from these light curves (\texttt{stingray.Eventlist.simulate\_times()}). 
...
Therefore, we initially used the \citet{timmer1995} method to get light curves with a very high count rate - a way to reduce the relative importance of the Poisson noise because of the square root involved - then renormalized the light curves to the wanted (lower) count rate and rms and finally used these light curves to simulate event lists.
...
%\begin{figure*}
%\gridline{\fig{imgs/pds_pdf}{0.5\textwidth}{(b)}
%          \fig{imgs/cosp_pdf}{0.5\textwidth}{(c)}}
%
%\caption{(b) Distribution of powers in the periodogram, before the FAD correction and after, shown as a histogram.
...
%\end{figure*}
\begin{figure*}
\gridline{\fig{imgs/before_and_after.eps}{\textwidth}{(a)}}
\gridline{\fig{imgs/pds_pdf.pdf}{\textwidth}{(b)}}
\caption{(Top) Periodogram and cospectrum, before and after FAD correction, for a pure white noise light curve. 
...
(Bottom) PDF of non-averaged powers in the cospectrum (red) and the periodogram (black), before the FAD correction and after, shown as a histogram. 
...
Then, we applied a dead time filter to the event list and applied the FAD correction, as follows:
\begin{enumerate}
\item split the two light curves in segments of 128 to 512 seconds
\item for each pair of light curve segments: 
	\begin{itemize}
	\item calculate the Fourier transform of each channel separately, and then of the summed channels (hereafter the \textit{total-intensity} light curve);
	\item multiply the Fourier amplitudes by $\sqrt{2/N_{ph}}$ in order to obtain Leahy-normalized periodograms;
	\item subtract the Fourier amplitudes of the two channels between them, \textit{take the absolute value}, and obtain this way the Fourier Amplitude Differences (FAD);
	\item \textit{smooth} the FAD using a Gaussian-window interpolation with a width of 1-2 seconds;
	\item use the separated single-channel and total-intensity Fourier amplitudes to calculate the periodograms;
...
Then, we fitted all spectra with the model which produced the simulated data and checked if the values were consistent with the input model parameters and the fit on the dead time-free periodogram.
Finally, we verified if \textit{at low frequencies} the total rms of the FAD-corrected spectra was consistent with the total rms of the dead time-affected cospectrum \textit{times the ratio between incident and detected photons}, using the count rates before and after applying the dead time filter.
...
We find that the FAD correction consistently overestimates the integrated rms when the count rate and rms are \textit{both} very high, \textit{independently from the frequency}.
This latter feature is less intuitive, because we would expect that this overestimation should somehow be more relevant where the correction is stronger, while this is not the case.
But for this very reason, it is somewhat easier to account for this through simulations, without having to make further risky frequency-dependent corrections to the rms.
At standard count rates and fractional rms for bright black hole binaries, $\sim$200 ct/s and 30\% rms, the difference is below 5\%. 
However, it increases \textit{linearly} with the count rate and \textit{quadratically} with rms.
A practical way to estimate this effect during analysis is to apply the FAD, obtain a best fit model, calculate the rms, then use the codes we shared with the Jupyter notebooks to simulate a number of realizations of the code, evaluating what is the amount of overestimation involved.
