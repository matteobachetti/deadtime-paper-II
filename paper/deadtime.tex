...
%We used the scripts contained in \texttt{HENDRICS} to load the event lists for the two detectors FPMA and FPMB, calibrate them to translate the PI detectors into energy values, and produced light curves with photons from 3 to 79 keV and binned with uniform time bins of 1/4096$\sim2.44\cdot10^{-4}$\,sec%
...
\begin{figure*}
    ...
    As can be seen from the best-fit curves, there is no significant difference between FAD-normalized and deadtime-free periodograms and cospectra (the curves overlap exactly).
...
\begin{figure*}
    ...
    clearly truer when the incident rate \textit{and} the \rms are high, and the frequency relatively low (as can be seen by the fact that the distribution of black points in both plots is more skewed towards positive values). }
    \label{fig:errors}
\end{figure*}
