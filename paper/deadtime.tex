...
%\begin{figure*}
%\gridline{\fig{rn_fourierdiff.eps}{0.33\textwidth}{(a)}
%	\fig{wn_fourierdiff.eps}{0.33\textwidth}{(b)}
%	\fig{fourier_vs_diff_dt.eps}{0.33\textwidth}{(c)}}      
%\caption{(a): Real-valued Fourier amplitudes derived from many evenly sampled light curves of $500\,\mathrm{s}$ duration with strong $1/f$ red noise (top, in black). On the bottom, we show the \textit{difference} between real-valued Fourier amplitudes as they would be sampled from two independent detectors using the same light curves as in the top panel. In red, we show the the frequency-dependent spread of the distributions, measured by the mean of the absolute values of the curves in each frequency bin. Panel (b) shows the same as panel (a), but using white noise light curves of $5\,\mathrm{s}$ duration affected by dead time. In the case of red noise without dead time, the difference in Fourier amplitudes effectively follows a white noise distribution. Because dead time affects the observed light curves \textit{in the detector} (while red noise is intrinsic to the source), the difference in Fourier amplitudes will still contain the dead time contribution to the Fourier amplitudes, and can thus be used to effectively separate real variability from spurious dead time effects.
%(c) Scatter distribution of the (averaged over multiple intervals) absolute values of dead time-affected Fourier amplitudes versus the difference of Fourier amplitudes for case (b): their relation is clearly linear, with a factor $1/\sqrt{2}$. \DHN{I don't really understand what (c) adds. Also, why 500s segments in (a) and 5s segments in (b)?}}
...
\item for each pair of light curve segments: 
	\begin{itemize}
	\item calculate the Fourier transform of each channel separately, and then of the summed channels (hereafter total-intensity); \DHN{You seem to use ``channel'' and ``detector'' interchangeably. Could people get confused when they think channel means energy channel? Just something to be careful with!}
	...
	\item use the unnormalized Fourier amplitudes from channels A and B to calculate the cospectrum; \DHN{These are kind of two different outcomes, right? I mean, you don't *need* to also calculate the normalized cospectrum, do you? Maybe leave this out and mention that you could do the same process with the cospectrum?}
	\item divide all periodograms and the cospectrum by the smoothed and squared FAD, and multiply by 2.
	\end{itemize}
\item normalize the periodograms to the wanted normalization (Leahy or fractional \rms \DHN{Probably want to put citations for fractional rms normalization here rather than further below?}).
\end{enumerate}
...
\begin{figure*}
    ...
    For comparison, we also plot the cospectrum without FAD (pink), showing very different amplitudes for the four Lorentzians, due to dead time. \DHN{This plot has too much in it. I have a hard time separating out which line is which even knowing what this should show. I would just plot the dead-time free PDStot and FAD-normalized total PDS (for which, by the way, you can't really read the label in the legend. Also, your model components don't seem to be included in the legend? Also, maybe cut this off at 1000 Hz, since there's only more white noise above?}
    }
    \label{fig:pds}
\end{figure*}
...
An example of this analysis is shown in Figure~\ref{fig:pds}. \DHN{I feel like Figure 3 requires a bit more text here. At least a couple of sentences along the lines of ``as we show in Figure 3, the FAD successfully corrects periodograms and cospectra when compared to dead-time free simulated spectra.''}
...
The simulations show that the shape of the periodogram is precisely corrected by the FAD procedure if (1) \textit{the input light curves have the same count rate} and (2) \textit{for values of the input count rate and \rms that are not too extreme}. Differing input count rates in different detectors matter in practice only for the single-channel periodogram, but \textit{not} for cospectra and total-intensity periodograms. 
At high count rates, single-channel periodograms are corrected very well only if the two channels have very similar count rates, and count rates must be more similar athigher count rates in order for the correction to apply.
However, we find that the total-intensity periodogram and the cospectrum remain well corrected by the FAD even if the count rate in the two channels differs by 30\% in most cases.
Therefore, we recommend to use the FAD very carefully with single-channel periodograms, which should not be an issue given that the total-intensity periodogram is more sensitive and more convenient to use. 
...
At $\sim$200 ct/s and 30\% \rms, the relative error is below 5\% (meaning that if the true \rms is 30\%, the measured \rms is between 30 and 32\% \DHN{So shouldn't this be something like 28 to 32 \% if it's symmetrical about zero or am I missing something here?}) and it is symmetrically distributed around 0, as expected from statistical uncertainties. 
At higher incident rates and \rms, the uncertainty distribution is biased towards positive relative errors, implying an overestimation of the \rms. \DHN{This is currently impossible to discern from the plot. Can we improve that plot somehow? I think partly there's so many circles overplotted that it all becomes hard to distinguish.}
